...
#define kSelfWidth  CGRectGetWidth(self.bounds)
#define kSelfHeight CGRectGetHeight(self.bounds)
...
- (void)drawTheUnderlayCircle:(CGContextRef)context {
    ...
    CGContextAddArc(context, kSelfWidth / 2, kSelfHeight / 2, _radius, -M_PI, M_PI, 0);
...
- (void)drawTheUnfilledArc:(CGContextRef)context {
    ...
    CGContextAddArc(context, kSelfWidth / 2, kSelfWidth / 2,
                    ...
                    DegreesToRadians(kCircleDegreeMax), 1);
...
- (void)drawTheGradient:(CGContextRef)context {
    UIGraphicsBeginImageContext(CGSizeMake(kSelfWidth, kSelfHeight));
    CGContextRef imageCtx = UIGraphicsGetCurrentContext();
    
    CGContextAddArc(imageCtx, kSelfWidth / 2, kSelfHeight / 2, _radius,
                    DegreesToRadians(kCircleDegree360 - kCircleDegreeMin),
                    DegreesToRadians(_angle), YES);
    CGContextSetLineCap(imageCtx, kCGLineCapRound);
    CGContextSetLineWidth(imageCtx, _borderWidth);
    ...
    // Gradient
    CGFloat components[8] = {52/255.0, 1.0, 58/255.0, 1.0,
        95.0/255.0, 100.0/255.0, 250/255.0, 1.0};
    CGColorSpaceRef baseSpace = CGColorSpaceCreateDeviceRGB();
    CGGradientRef gradient = CGGradientCreateWithColorComponents(baseSpace, components, 0, 2);
...
//    NSLog(@"currentAngle = %f", currentAngle); // 140 -> 360 -> 40
    ...
    if (currentAngle < kCircleDegreeMin - offsetAngle &&
        currentAngle > kCircleDegreeMax + offsetAngle) return NO;
